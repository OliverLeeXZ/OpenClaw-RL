...
from openclaw_self_opd_api_server import OpenClawSelfOPDAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenClawSelfOPDAPIServer(
            args=args,
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[OpenClawSelfOPDWorker] waiting for self-opd samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[OpenClawSelfOPDWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_openclaw_self_opd(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        extra_metrics = {"rollout/self_opd_success_rate": sum(eval_scores) / len(eval_scores)}
        print(
            f"[OpenClawSelfOPDWorker] success_rate={extra_metrics['rollout/self_opd_success_rate']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
